...
def lilypond_minor_version():
    ...
    if cmd:
        ...
        _lilypond_minor_version = 22  # assume 2.22 if we can't figure it out
    return _lilypond_minor_version
...
def lilypond_command():
    if hasattr(shutil, "which"):
        ...
        ] + placesToTry  # if renamed from the above (try specific versions 1st, in case default is older)
        placesToTry += [
...
three_dots = "\u22ee"
if not type("") == type(""):
    three_dots = three_dots.encode("utf-8")  # Python 2
...
def all_scores_start(inDat):
    if lilypond_minor_version() < 20:
        errExit(
            "Lilypond 2.18 and below is no longer supported"
        )  # 2.20 is in Ubuntu 20.04 LTS and 2.22 can be installed on a 2011 Mac; if you're stuck on an older EOL'd distro without Internet, use jianpu-ly 1.825 or earlier
    r = r"""\version "2.%d.0"
#(set-global-staff-size %g)""" % (
        (22 if "g[" in inDat or "]g" in inDat else 20),
        staff_size,
    )
    r += r"""
...
#(define-music-function
   ...
   #})"""
    ...
    if re.search(r"(\s|^)(angka|Indonesian)(\s|$)", inDat):
        r += r"""
...
def score_end(**headers):
    ...
    if not lyric_size == staff_size:
        ...
        if lSize > 3:
            sys.stderr.write(
                "WARNING: potential layout problems; consider increasing j2ly_staff_size to be closer to j2ly_lyric_size\n"
            )  # TODO: is 3 a good threshold for this warning?  (need to check different Lilypond versions)
        layoutExtra = (
            r" \override Lyrics.LyricText.font-size = #"
            + ("+" if lSize >= 0 else "")
            + str(lSize)
            + " "
        )
    if notehead_markup.noIndent:
        layoutExtra += " indent = 0.0 "
    if notehead_markup.raggedLast:
        layoutExtra += " ragged-last = ##t "
    if notehead_markup.noBarNums:
        layoutExtra += r' \context { \Score \remove "Bar_number_engraver" } '
    if notehead_markup.chordsRoman:
        layoutExtra += r"\context { \ChordNames \consists #(lambda (cx) (let ((tonic #{ c #})) (make-engraver ((initialize engraver) (set! (ly:context-property cx 'chordRootNamer) (lambda (pitch capitalized) (let ((degree (1+ (ly:pitch-notename (ly:pitch-diff pitch tonic))))) (number-format 'roman-upper degree))))) (listeners ((key-change-event engraver event) (set! tonic (ly:event-property event 'tonic))))))) } "  # based on a lists.gnu.org snippet
    if midi:
        ret += r"\midi { \context { \Score tempoWholesPerMinute = #(ly:make-moment 84 4)}}"  # will be overridden by any \tempo command used later
    else:
        ret += (
            r"\layout{"
            + layoutExtra
...
def jianpu_voice_start(isTemp=0):
    if not isTemp and maxBeams >= 2:
        stemLenFrac = "0.5"  # sometimes needed if the semiquavers occur in isolation rather than in groups (TODO do we need to increase this for 3+ beams in some cases?)
    else:
        stemLenFrac = "0"
    voiceName = uniqName()
    ...
    \override TupletBracket #'bracket-visibility = ##t""" % (
        stemLenFrac,
        (
            r"\override Beam.after-line-breaking = #flip-beams"
            if inner_beams_below
            else ""
        ),
    )
...
def jianpu_staff_start(inst=None):
    ...
    if notehead_markup.withStaff:
        inst = None  # we'll put the label on the 5-line staff (TODO: use StaffGroup or something?)
    if not_angka:
        r = r"""
...
"""  # (whether this is needed or not depends on Lilypond version; 2.22 puts more space than 2.18,2.20.  Must set higher than 5, which sometimes gets collisions between beams in 2.20)
    ...
    if notehead_markup.separateTimesig:
        r += r"\override Staff.TimeSignature #'stencil = ##f" + "\n"
    return r, voiceName
...
if __name__ == "__main__":
    if (
        "--noRestHack" in sys.argv
    ):  # TODO: document (this is a debug option you might want to try if things are going wrong, but unlikely to still be needed)
        use_rest_hack = False
        sys.argv.remove("--noRestHack")
    if (
        "--nosort" in sys.argv
    ):  # TODO: document (this is a hack for if someone's incorrectly coded 2-voice music as chords and they want to cross the parts)
        sort_chords = False
        sys.argv.remove("--nosort")
assert not (
...
class NoteheadMarkup:
    ...
    def initOneScore(self):
        ...
        self.inBeamGroup = self.lastNBeams = self.onePage = self.noBarNums = (
            self.chordsRoman
        ) = self.noIndent = self.raggedLast = self.withStaff = 0
        self.keepLength = 0
        ...
        self.current_accidentals = (
            {}
        )  # used to predict whether Lilypond will draw the accidental or not, for beam spacing purposes
        self.barNo = 1
        self.tuplet = (1, 1)
    ...
    def endScore(self):
        if self.barPos == self.startBarPos:
            ...
            errExit(
                "Incomplete bar at end of score %d (%g beats)"
                % (scoreNo, self.barPos * 1.0 / self.beatLength)
            )
    ...
    def setAnac(self, denom, dotted):
        ...
        if self.barPos < 0:
            errExit(
                "Anacrusis is longer than bar in score %d" % scoreNo
    ...
    def __call__(self, figures, nBeams, dots, octave, accidental, tremolo, word, line):
        ...
        if len(figures) > 1:
            if accidental and not_angka:
                scoreError(
                    "Accidentals in chords not yet implemented in Indonesian not-angka mode:",
                    word,
                    line,
                )
            if "0" in figures:
            ...
            if "x" in figures:
                scoreError("Can't have percussion beat in chord:", word, line)
        self.notesHad.append(figures)

        isChord = len(figures) > 1
        if isChord:
            chord_ret, octave, placeholder_chord = chordNotes_markup(
                re.sub(r"[\\qsdh.]", "", word), word, line
            )  # word w/out durations
        ...
        if invisTieLast:  # (so figures == "-")
            if (
                ...
                if (
                    ...
                    sys.stderr.write(
                        "Warning: jianpu barline-crossing tie won't be done right because your Lilypond version is older than 2.20\n"
                    )
                if self.barPos == 0:
                    tremolo = self.last_tremolo
            if (
                self.current_chord
            ):  # a chord is currently in progress, and we're extending it with a tie
                isChord = True
                chord_ret, octave, placeholder_chord = chordNotes_markup(
                    re.sub(r"[\\qsdh.]", "", self.current_chord), word, line
                )
                if not midi and not western:
                    placeholder_chord = placeholder_chord[:-1].split()[
                        -1
                    ]  # just have one note of it for dashes
            ...
            self.octavesSeen.append(
                {",,,": -3, ",,": -2, ",": -1, "": 0, "'": 1, "''": 2, "'''": 3}[octave]
            )  # for figuring out the best clef with WithStaff
            self.last_tremolo = tremolo
            if isChord:
                self.current_chord = word
            else:
                self.current_chord = None
        if not figures == "-":
            self.last_figures = figures
        if not isChord and not accidental in ["", "#", "b"]:
        ...
        if (
            nBeams and not midi and not western
        ...
        for figure in list(figures):
            if "1" <= figure <= "7":
                ...
                self.current_accidentals[octave][
                    int(figure) - 1
                ] = accidental  # TODO: not sensible (assumes accidental applies to EVERY note in the chord, see above)
        inRestHack = replaceLast = 0
        if not midi and not western:
            if ret:
                ret = ret.rstrip() + "\n"  # try to keep the .ly code vaguely readable
            if octave == "''" and not invisTieLast:
                ret += r"  \once \override Score.TextScript.outside-staff-priority = 45"  # inside bar numbers etc
            ...
            if self.rplacNextIfStillInBeam and leftBeams and nBeams:
                replaceLast = (
                    self.rplacNextIfStillInBeam
                )  # didn't need the rest-hack here after all
            self.rplacNextIfStillInBeam = None
            if (
                placeholder_chord == "r"
                and use_rest_hack
                and nBeams
                and not (leftBeams and not not_angka)
            ):
                placeholder_chord = "c"
                ...
                if (
                    has_lyrics and not self.withStaff
        ...
        if placeholder_chord.startswith("<"):
            ret += placeholder_chord  # chord in western or midi
        elif not isChord or figures.startswith("-"):  # single note or rest
            if (
                need_space_for_accidental
                and not not_angka
                and not (figures.startswith("-") or midi or western)
            ):
                ret += r"\once \tweak Accidental.extra-offset #'(0 . 0.7)"
            ret += placeholder_chord
            if midi or western or not not_angka:
                ret += {"": "", "#": "is", "b": "es"}[accidental]
            if (midi or western) and not placeholder_chord == "r":
                ret += {
                    "": "'",
                    "'": "''",
                    "''": "'''",
                    ...
                    octave
                ]  # so no-mark starts near middle C
            if add_cautionary_accidental:
                ret += "!"
        ret += ("%d" % length) + dots
        if self.rplacNextIfStillInBeam:
            self.rplacNextIfStillInBeam += ("r%d" % length) + dots + "["
        if tremolo:
            if midi or western:
                if (
        ...
        if not midi and not western and not "-" in figures:
            if not nBeams:
                ...
                oDict = {
                    ",": r"-\tweak #'Y-offset #%.1f "
                    % (grace_height - 1 - nBeams * 0.3),
                    ",,": r"-\tweak #'Y-offset #%.1f "
                    % (grace_height - 1.6 - nBeams * 0.3),
                    ",,,": r"-\tweak #'Y-offset #%.1f "
                    % (grace_height - 2 - nBeams * 0.3),
                }
                ret += oDict.get(octave, "")
                ...
                oDict.update(
                    {
                        ...
                        "'''": r"-\tweak #'extra-offset #'(0.4 . 4.3) -\markup{\bold "
                        + three_dots
                        + "}",
                    }
                )
        ...
        if (
            nBeams
            and (
                not self.inBeamGroup
                or (self.inBeamGroup == "restHack" and not replaceLast)
                or inRestHack
            )
            and not midi
            and not western
        ...
        if self.graceType and self.barPos == self.barLength:
            ...
            if is_isolated_note:
                ...
                if self.graceType == "before":
                    ret = r"s%d [ \jianpuGraceCurveEnd %s" % (
                        length,
                        ret.replace("[", ""),
                    )
                else:
        ...
        if (
            self.barPos % self.beatLength == 0 and self.inBeamGroup
        ...
        self.unicode_approx.append(
            {"#": "\u266f", "b": "\u266d"}.get(0 if invisTieLast else accidental, "")
        ...
        if invisTieLast:
            if midi or western:
                if not (tremolo and placeholder_chord.startswith("<")):
                    aftrlast = " ~"
            elif not tremolo:
        ...
        if figures == "x" and western:
            ret = (
                r"\once \override NoteHead.style = #'cross \once \override NoteHead.no-ledgers = ##t "
                + ret
            )
        if inRestHack:
            ret += " } "  # end temporary voice for the "-" (non)-note
        elif tieEnd:
            ret += " " + tieEnd  # end of JianpuTie curve
        return (
            aftrLastNonDash,
            figures == "-",
            b4last,
...
def parseNote(word, origWord, line):
    ...
    if re.match(r"[\\]+$", nBeams):
        nBeams = len(
            nBeams
        )  # requested by a user who found British note-length names hard to remember; won't work if the \ is placed at the start, as that'll be a Lilypond command
    elif nBeams:
        try:
    ...
    if len(octaves) > 1 and len(figures) == 1:
        scoreError(
            "Multiple octaves should not applied to a single note:", origWord, line
        )
    if octaves:
        octave = octaves[0]
    else:
    ...
    if (
        len(figures) > 1
    ):  # octave + accidental dealt with separately BUT still need to keep one for the beaming and need_space_for_accidental logic (TODO actually current_accidentals needs rewriting for chords, but this works in most cases for now)
...
def write_docs():
    ...
    def htmlify(l):
        if "--html" in sys.argv:
            return (
                ...
                .replace("approximation", "approx&shy;imation")
                .replace("instrument=Flute", "instrument=<wbr>Flute")
                .replace("automatically", "automat&shy;ically")
                .replace("SeparateTimesig", "Separate&shy;Timesig")
            )  # not sure about that last one because we don't want to hide that it's 1 word, but we do want Chrome on small devices in not-so-small print to fit it on a line rather than zoom out the whole page
        else:
            return l
    ...
    for line in __doc__.split("\n"):
        ...
        if hasNonAscii ^ ("--chinese" in sys.argv):
            continue
...
def write_version():
    ...
    for l in __doc__.split("\n"):
        if l.startswith("# v"):
            versions.append(
                (
                    float(l[len("# v") : l.index(" ", len("# v"))]),
                    l.replace("#", "jianpu-ly", 1) + "\n\n",
                )
            )
...
def xml2jianpu(x):
    ...
    def s(name, attrs):
        ...
        if name == "measure":
            oldBarsig = barSig[0]
            barSig[0] = keySig[0][:]
            if barSig[1] is not None:
                barSig[0][barSig[1]] = oldBarsig[barSig[1]]  # for tie
    ...
    def e(name):
        d0 = dat[0].strip()
        if (
            name in ["work-title", "movement-title"]
            or name == "credit-words"
            and dat[1].get("justify", "") == "center"
        ):
            if not (any(r.startswith("title=") for r in ret)):
                ret.append("title=" + d0.replace("\n", " "))
        elif (
            name == "creator"
            and dat[1].get("type", "") == "composer"
            or name == "credit-words"
            and dat[1].get("justify", "") == "right"
        ):
            ...
            for n, p in enumerate(partsInProgress):
                ...
                if (
                    positionsInProgress[n] < max(positionsInProgress)
                    and positionsInProgress[n] in paddingRestDict
                ):
                    p.append(
                        " ".join(
                            paddingRestList[paddingRestDict[positionsInProgress[n]] :]
                        )
                    )
                else:
            ...
            del partsInProgress[:]
            del positionsInProgress[:]
            positionsInProgress.append(0)
            partsInProgress.append([])
            mxlPosition[0] = mxlPosition[1] = 0
            del paddingRestList[:]
            ...
            keySig[0] = [""] * 7
            if d0.startswith("-"):
                keyAcc, start, inc = "b", 4 - 1, 4  # Bb (b)4
            else:
                keyAcc, start, inc = "#", 7 - 1, 3  # F# (#)7
            ...
            for n, p in enumerate(partsInProgress):
                if positionsInProgress[n] == max(positionsInProgress):
                    ...
                    tSig[0].append(
                        None
                    )  # and hope anacrusis is fixed in paddingRestList before time signature gets copied to this part (TODO in theory this might not happen with all MusicXML generators)
        elif name == "duration":
            mxlPosition[1] = int(
                dat[0].strip()
            )  # last duration (could be inside note or backup,forward: handle when close)
        elif name == "backup":
            ...
            if not tSig[1] == int(time[0]) * 8 / int(time[1]):
                ...
                for n, p in enumerate(tSig[0][1:]):
                    if not p is None:
                        partsInProgress[n][p] += a
            tSig[0] = None
        elif name == "beat-unit":
            tempo[0] = typesMM.get(name, "4")
        elif name == "beat-minute" or name == "per-minute":
            tempo[1] = d0
        elif name == "metronome":
            if tempo[0] and tempo[1]:
                for n, p in enumerate(partsInProgress):
                    if positionsInProgress[n] == max(positionsInProgress):
                        ...
                        break
            tempo[0] = tempo[1] = (
                ""  # for now we ignore <metronome> elements that don't specify all parameters
            )
        elif name == "step":
            note[0][0] = d0
        elif name == "rest":
            ...
            note[0][5] += {"start": " (", "stop": " )"}[dat[1].get("type", "")]
        elif name == "tie":
            note[0][6] = {"start": "~", "stop": ""}[dat[1].get("type", "")]
        elif name == "actual-notes":
            note[0][7] = d0
        elif name == "tuplet":
            note[0][8] = dat[1].get("type", "")
        elif name == "chord":
            note[0][9] = True
        elif name == "grace":
            note[0][10] = True
            ...
            name
            in "ppppp pppp ppp pp p mp mf f ff fff ffff fffff fp sf sfz n rfz".split()
        ):
            note[0][5] += " \\" + name
        elif name == "words":
            toAdd = r' ^"' + dat[0].strip().replace('"', "'") + '"'
            if not toAdd in note[0][5]:
                note[0][5] += toAdd
        elif name == "note":
            ...
            if ourRet is None:
                for i, p in enumerate(positionsInProgress):
                    if (
                        p < mxlPosition[0] and p in paddingRestDict
                    ):  # match but need padding
                        ourRet, ourI = partsInProgress[i], i
                        ourRet.append(
                            " ".join(
                                paddingRestList[
                                    paddingRestDict[p] : paddingRestDict[mxlPosition[0]]
                                ]
                            )
                        )  # TODO: collapse to whole-bar rests when needed (low priority because this should not happen often)
                        positionsInProgress[i] = mxlPosition[0]
                        break
            if ourRet is None:  # need new part
                partsInProgress.append(
                    paddingRestList[: paddingRestDict[mxlPosition[0]]]
                )  # TODO: collapse to whole-bar rests when needed (as above)
            ...
            step, octave, acc, nType, dot, extras, tie, tuplet, tState, chord, grace = (
                note[0]
            )
            note[0] = [""] * 11
            if step == "r":
                ...
                while dTone > 6:
                    r += "'"
                    dTone -= 7
                acc = barSig[0][dTone % 7] = {
                    "flat": "b",
                    "sharp": "#",
                    "natural": "",
                }.get(acc, barSig[0][dTone % 7])
                barSig[1] = (dTone % 7) if tie else None
                if keySig[0][dTone % 7] == "#":
                    acc = "" if acc == "#" else "b"
                if keySig[0][dTone % 7] == "b":
                    acc = "" if acc == "b" else "#"
            if chord:
                rr = prevChord[1][prevChord[0]]
                prevChord[1][prevChord[0]] = (
                    rr.split()[0] + r + "".join([" " + x for x in rr.split()[1:]])
                )
                return
            if tState == "start":
            ...
            if not nType:
                ...
                if nn:
                    ...
                    if nn:
                        ...
                        assert (
                            0
                        ), "Full-measure note or rest at unrecognised bar length"  # will probably need to split
            if (
                not tSig[0] == None and ourI == 0
            ):  # we're counting the length of the first bar, for anacrusis
            ...
            r += acc + d[nType] + " "
            if ourI == 0:
                paddingRestList.append(
                    "0" + d[nType]
                )  # we hope the subsequent voices are not cross-rhythm with the first voice, at least not at points where <backup> and <forward> occur
    ...
    ret = "\n".join(ret)
    if not type("") == type(ret):
        ret = ret.encode("utf-8")  # Python 2
    return ret
...
def graceNotes_markup(notes, word, line, isAfter, harmonic=False):
    if lilypond_minor_version() < 22:
        errExit(
            "grace notes requires Lilypond 2.22+, we found 2."
    ...
    mr = []
    if isAfter:
        mr.append(r"\once \override Score.JianpuGraceCurve.direction = #LEFT ")
    ...
    for i in xrange(len(notes)):
        ...
        if n == "#":
            ...
            if harmonic:
                mr[-1] += r" \flageolet "  # deal with harmonic articulations
            accidental = ""
...
def grace_octave_fix(notes, word, line):
    ...
    def gof_inner(notes):
        return re.sub(
            "^(.*)([1-9])([^1-9]+)$",
            lambda m: gof_inner(m.group(1)) + chr(0) + m.group(3) + m.group(2),
            notes,
        )  # the chr(0) is a temporary marker for below

    n2 = gof_inner(notes)

    def find_ambiguous_octaves(n):
        return re.sub(
            "^[^',1-9]*[',]+[^',1-9]*[1-9](.*)$",
            lambda m: find_ambiguous_octaves(m.group(1)),
            n,
        )
    ...
    if re.search("[,']", ambiguous_part):
        if notehead_markup.octavesPosition == "after":
            ap2 = re.sub(
                r"([1-9][^1-9,']*)([,']+)", r"\2\1", ambiguous_part
            )  # after to before (no recursion needed)
            ...
            if not ambiguous_part == notes:
                msg += " in the " + ambiguous_part + " part of"
            else:
                msg += ":"
...
def chordNotes_markup(notes, word, line):
    notes = grace_octave_fix(
        notes, word, line
    )  # ensures octaves and accidentals come before notes
    accidental = ""
    figure = ""
    octave = ""
    sortKey = 0
    ...
    for f in dNotes:
        placeholder_chord += (
            placeholders[f["figure"]] + {"": "", "#": "is", "b": "es"}[f["accidental"]]
        )
        if "," in f["octave"]:
            placeholder_chord += f["octave"][:-1] + " "
        else:
            placeholder_chord += f["octave"] + "' "
    placeholder_chord += ">"
    ...
    for f in dNotes:
        ...
        ret += (
            ...
            + f["figure"]
            + '" '
            + placeholders[f["figure"]]
            + {"": "", "#": "is", "b": "es"}[f["accidental"]]
        )
        if "," in f["octave"]:
            ret += f["octave"][:-1] + " "
        else:
            ret += f["octave"] + "' "
        if "," in f["octave"]:
            ret += r"\tweak #'Y-offset #%.1f " % (
                baseline - 0.1 - 1.2 * offsets[f["octave"]]
            )
        elif "'" in f["octave"]:
            ret += r"\tweak #'Y-offset #%.1f " % (
                baseline + 1.6 + 0.02 * offsets[f["octave"]]
            )
        ret += oDict[f["octave"]] + " "
        baseline += 2
        if "'" in f["octave"]:
...
note_regex = (
    ...
    + r"[0-9x.,'cqsdh\\#b-]*"
)  # and other figures or attrs after
...
def getLY(score, headers=None, have_final_barline=True):
    ...
    score = re.sub(
        ...
        lambda m: "\n" + " ".join(m.group().split()),
        score,
    )  # this one DOES apply to lyrics etc: if newline immediately after, collapse until next double newline
    for line in score.split("\n"):
        line = fix_fullwidth(line).strip()
        line = re.sub(
            r"^%%\s*tempo:\s*(\S+)\s*$", r"\1", line
        ...
        if line.startswith("LP:"):
            ...
            escaping = 0
            if line.replace(":LP", "").strip():
                sys.stderr.write(
                    "Warning: current implementation ignores anything after :LP on same line\n"
                )  # TODO
        elif escaping:
            out.append(line + "\n")
        elif not line:
            ...
            if not headers.get(hName, hValue) == hValue:
                if hName == "instrument":
                    ...
                    missing = "NextScore"
                errExit(
                    "Changing header '%s' from '%s' to '%s' (is there a missing %s?)"
            ...
            # have around anyway:
            line = re.sub(
                r"(?<=\s)(g\[[#b',1-9qsdh]+\]\s*)+g\[([#b',1-9qsdh]+)\](?=\s)",
                lambda m: re.sub(r"\]\s*g\[", "", m.group()),
                line,
            )
            ...
            line = re.sub(
                r"((?:^|\s)" + note_regex + r")([()~]+)(?=\s|$)",
                lambda m: " ".join([m.group(1)] + list(m.group(2))),
                line,
            )
            for word in line.split():
                ...
                if re.match("[16]=[#b][A-Ga-g]$", word):
                    word = (
                        word[:2] + word[3] + word[2]
                    )  # somebody wrote a key name backwards (bE instead of Eb), we can fix that here
                # -----------------------------------
                ...
                if word.startswith("%"):
                    ...
                    if midi or western:
                        ...
                        if transposeFrom == "c" and transposeTo[0] in "gab":
                            transposeTo += ","
                        if transposeFrom == "a" and transposeTo[0] in "cd":
                            transposeTo += "'"
                        out.append(
                            r"\transpose "
                            + transposeFrom
                        ...
                        out.append(
                            r"\mark \markup{%s}"
                            % word.replace("b", r"\flat").replace("#", r"\sharp")
                        )
                    ...
                    finger = {
                        ...
                        "4": "\u56db",
                        "souyin": "\u4e45",  # jiu3
                        "harmonic": "\u25cb",  # white circle: TODO: can we use Lilypond's ^\flageolet command (not in a \finger{}) which doesn't require a font with 25CB in it? or would that get wrong size? (can be tweaked)
                        "up": "\u2197",  # NE arrow
                        "down": "\u2198",  # SE arrow
                        "bend": "\u293b",  # bottom arc anticlockwise arrow
                        "tilde": "\u223c",  # full-width tilde.  Could also use U+1D008 "Byzantine musical symbol syrmatiki" but that (a) won't display on macOS (as of 12.6) and (b) needs special consideration for old versions of Python 2 on narrow Unicode builds
                    }.get(finger, finger)
                    if not type("") == type(""):
                    ...
                    if western or midi:
                        ...
                        out.append(jianpu_voice_start(1)[0])
                    if not midi:
                        out.append(r'\cadenzaOn \note-mod "(" r8 \cadenzaOff ')
                elif word == ")]":
                    if not midi:
                        if notehead_markup.barPos:
                            out.append(r'\cadenzaOn \note-mod ")" r8 \cadenzaOff ')
                        else:
                            out.append(
                                r"""
...
\context Score \applyContext #(lambda (ctx) (ly:context-set-property! ctx 'currentBarNumber (+ (ly:context-property ctx 'currentBarNumber) -1))) """
                    ...
                    if notehead_markup.separateTimesig and not midi:
                        out.append(
                            r"\mark \markup{" + word + "}"
                        )  # don't try to add RehearsalMark overrides here: it will go wrong during mark merging; we do that later
                    ...
                    if notehead_markup.chordsRoman:
                        sys.stderr.write(
                            "WARNING: Duplicate ChordsRoman, did you miss out a NextScore?\n"
                        )
                    ...
                    if notehead_markup.noIndent:
                        sys.stderr.write(
                            "WARNING: Duplicate NoIndent, did you miss out a NextScore?\n"
                        )
                    notehead_markup.noIndent = 1
                elif word == "RaggedLast":
                    if notehead_markup.raggedLast:
                        sys.stderr.write(
                            "WARNING: Duplicate Raggedlast, did you miss out a NextScore?\n"
                        )
                    notehead_markup.raggedLast = 1
                elif word == "SeparateTimesig":
                    if notehead_markup.separateTimesig and not midi and not western:
                        sys.stderr.write(
                    ...
                    if numBraces == 1:
                        notehead_markup.barPos = (
                            notehead_markup.barPos
                            + (newBarPos - oldBarPos) * extraRepeats
                        ) % notehead_markup.barLength
                    # TODO: update barNo also (but it's used only for error reports)
                elif word == "A{":
                    out[rStartP] = out[rStartP].replace(
                        "percent", "volta"
                    )  # for time-bars with 3 or more times, you can say R3{ ... } A{ ... } (TODO document this)
                    ...
                    repeatStack.append(
                        (numBraces, oldBarPos, extraRepeats + 1, rStartP)
                    )
                    out[rStartP] = out[rStartP].replace(
                        ("volta %d " % (extraRepeats + 1)),
                        ("volta %d " % (extraRepeats + 2)),
                    )  # ensure there's enough repeats for the alternatives
                elif (
                    word.startswith("\\")
                    or word.startswith("^\\")
                    ...
                    if (
                        ...
                        and not midi
                        and not western
                        and lastNonDashPtr < lastPtr
                    ):  # tie from the number, not the last dash
                        ...
                        if (
                            ...
                            sys.stderr.write(
                                "Warning: jianpu long-note tie won't be done right because your Lilypond version is older than 2.20\n"
                            )
                elif re.match(r"[1-9][0-9]*\[$", word):
                    # tuplet start, e.g. 3[
                    fitIn = int(word[:-1])
                    ...
                    if midi or western:
                        out.append(
                            r"\grace { "
                            + gracenotes_western(word[2:-1], word, line)
                            + " }"
                        )
                    else:
                        out.append(
                            r"\grace { "
                            + graceNotes_markup(word[2:-1], word, line, 0, isInHarmonic)
                            + " }"
                        )
                    ...
                    if midi or western:
                        out[lastPtr] = (
                            ...
                            + out[lastPtr]
                            + " } { "
                            + gracenotes_western(word[1:-2], word, line)
                            + " }"
                        )
                    else:
                        out[lastPtr] = (
                            ...
                            + out[lastPtr]
                            + " } { "
                            + graceNotes_markup(word[1:-2], word, line, 1, isInHarmonic)
                            + " }"
                        )
                elif word == "Fine":
                    need_final_barline = False
                    if lilypond_minor_version() >= 24:
                        out.append(r"\volta 2 \fine \volta 1")
                    else:
                        out.append(
                    ...
                    if lilypond_minor_version() < 24:
                        errExit("Need at least Lilypond 24 for Segno")
                    out.append(r"\repeat segno 2 {")
                elif word == "DS":
                    out.append(DS)
                elif word == "ToCoda":
                    if lilypond_minor_version() < 24:
                        errExit("Need at least Lilypond 24 for coda")
                    ...
                    if baseOctaveChange:
                        ...
                        if not word:
                            continue  # allow just < and > by itself in a word
                    figures, nBeams, dots, octave, accidental, tremolo = parseNote(
                        word, word0, line
                    )
                    ...
                    if replaceLast:
                        out[lastPtr] = replaceLast
                    if b4last:
                        out[lastPtr] = b4last + out[lastPtr]
                    if aftrlast:
                        out.insert(lastPtr + 1, aftrlast)
                    if aftrLastNonDash:
                        out.insert(lastNonDashPtr + 1, aftrLastNonDash)
                    lastPtr = len(out)
                    if not isDash:
                    ...
                    if aftrnext:
                        if need_space_for_accidental:
                            aftrnext = aftrnext.replace(
                                r"\markup", r"\markup \halign #2 ", 1
                            )
                        out.append(aftrnext)
                        aftrnext = None
                    ...
                    if isInHarmonic and not midi and not western and not figures == "-":
                        out[-1] += r" \flageolet "
    ...
    if have_final_barline and need_final_barline and not midi:
        out.append(r'\bar "|."')
    ...
    if (
        midi or western
    ):  # collapse/combine tied notes into longer notes (even in MIDI, for 2-note tremolo extension)
        for numNotes, dot, result in [
            ...
            out = re.sub(
                ...
                + dot
                + r'((?::32)?) +~(( \\[^ ]+| [_^]"[^"]*")*) '
                + " +~ ".join(["(?P=note)4" + dot] * (numNotes - 1)),
                r"\g<1>" + result + r"\g<2>\g<3>",
                out,
            )
            out = re.sub(
                "r4"
            ...
            out = re.sub(
                ...
                + " +~ ".join(["< (?P=note1) (?P=note2) >4" + dot] * (numNotes - 1)),
                r"\\repeat tremolo "
                + str(chkLen * numNotes)
                + r" { \g<1>32 \g<2>32 }\g<3>",
                out,
        ...
        out = re.sub(
            ...
            r"\g<1>\g<3>\g<2>",
            out,
        )  # dynamics need to attach inside the tremolo (but \bar doesn't)
        out = re.sub(
            r'(%\{ bar [0-9]*: %\} | \\major ) *r(?=[^ ]*(?: [_^]"[^"]*")?[| ]* (?:\\noPageBreak )?(?:%\{ bar|\\bar|\}$))',
            r"\g<1>R",
            out,
        ...
        )  # don't let it hang too far up in the air
    if not_angka:
...
def process_input(inDat):
    ...
    if unicode_mode:
        return (
            get_unicode_approx(re.sub(r"\sUnicode\s", " ", " " + inDat + " ").strip())
            + "\n"
        )
    ret = []
    ...
    inDat = re.sub(
        r"(%.*Next)(?=(Part|Score)\s)", r"\1 ", inDat
    )  # in case someone commented out NextScore or NextPart
    for score in re.split(r"\sNextScore\s", " " + inDat + " "):
        ...
        for midi in [False, True]:
            ...
            separate_score_per_part = (
                ...
                and len(parts) > 1
            )  # (results in 1st MIDI file containing all parts, then each MIDI file containing one part, if there's more than 1 part)
            for separate_scores in (
                ...
                for partNo, part in enumerate(parts):
                    ...
                    out, maxBeams, lyrics, headers = getLY(
                        part, headers, partNo == 0 or separate_scores
                    )  # assume 1st part doesn't have 'tacet al fine'
                    ...
                    if "chords" in headers:
                        if "frets" in headers:
                            ...
                            fretsInc = (
                                r'\include "predefined-' + frets + '-fretboards.ly"\n'
                            )
                            if not fretsInc in ret:
                                ret.insert(1, fretsInc)  # after all-scores-start
                            del headers["frets"]
                        else:
                            frets = None
                        ret.append(
                        ...
                        if frets:
                            ret.append(
                                ...
                                + (
                                    ""
                                    if frets == "guitar"
                                    else r"\set Staff.stringTunings = #"
                                    + frets
                                    + "-tuning"
                                )
                                + r" \chordmode { "
                    ...
                    if midi:
                        ...
                        if notehead_markup.withStaff:
                            ...
                            average_octave = (
                                sum(notehead_markup.octavesSeen)
                                * 1.0
                                / len(notehead_markup.octavesSeen)
                            )
                            if average_octave < -0.5:
                                staffStart += r" \clef bass"  # might want to say <0 but being conservative for now
                            ret.append(
                                staffStart
                                + " "
                                + getLY(part, have_final_barline=False)[0]
                                + " "
                                + western_staff_end()
                            )
                            western = False
...
def get_unicode_approx(inDat):
    ...
    has_lyrics = (
        True  # doesn't matter for our purposes (see 'false positive' comment above)
    )
    uniqCount = 0
    scoreNo = 1
    getLY(inDat, {})
...
(3) switch from Microsoft Windows to GNU/Linux"""
            ...
            if cmd:
                ...
                if sys.platform == "darwin":
                    ...
                    shutil.which("evince")
                    if hasattr(shutil, "which")
                    else os.path.exists("/usr/bin/evince")
                ):
